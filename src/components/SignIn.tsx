...
import type { SigninFn } from '../actions/signin';

export const SignIn = ({ signin }: { signin: SigninFn }) => {
  const [result, action, isPending] = useActionState<ReturnType<SigninFn>, FormData>(
    (_, payload) => signin(payload),
    null
  );
  ...
  return (
    <form action={action}>
      ...
      <button
        ...
        disabled={disabled}
      >
        Sign In {isPending ? <span className="ml-2 animate-spin">↻</span> : null}
      </button>
